...
Customers at the restaurant use a tablet device to orders dishes. When an order is placed, kitchen staff are notified and start preparing the dish. Once the dish is ready to be served, it's swiftly delivered to the customer via a robotic sushi train. When the customer chooses to 'end' their meal the tablet displays the total cost and prints out a slip they can present to the cashier. Restaurant policy dictates that any dish which takes longer than 30 seconds to serve is _free_.
